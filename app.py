...
# ==========================================
# Initialize Session States (Full Tasks List Restored)
# ==========================================
...
# ==========================================
# TAB 3: Employee Management (Camera Removed, Smaller Photos)
# ==========================================
with tab3:
    ...
    with st.expander("➕ අලුත් සේවකයෙක් ඇතුලත් කරන්න", expanded=True):
        st.info("💡 ගැලරියෙන් පින්තූරය තෝරන්න.")
        ...
        emp_photo = st.file_uploader("පින්තූරය තෝරන්න (Gallery):", type=["jpg", "png", "jpeg"], key="new_emp_file")
        
        if emp_photo is not None:
            st.success("✅ පින්තූරය සාර්ථකව ලෝඩ් විය!")
            st.image(emp_photo, width=120, caption="Preview")
            
        if st.button("සේවකයා සේව් කරන්න", type="primary"):
            if emp_name:
    ...
    if not emps_df.empty:
        ...
        if edit_emp_name != "තෝරන්න (Select)":
            ...
            if curr_photo:
                ...
                st.image(curr_photo, width=120)
            else:
                st.info("දැනට පින්තූරයක් නැත.")
                
            upd_photo = st.file_uploader("අලුත් පින්තූරයක් දානවා නම් තෝරන්න:", type=["jpg", "png", "jpeg"], key=f"upd_file_{emp_id}")
                
            if upd_photo is not None:
                st.success("✅ අලුත් පින්තූරය සාර්ථකව ලෝඩ් විය!")
                st.image(upd_photo, width=120, caption="New Preview")
    ...
    # --- View Employees List (Smaller Photos) ---
    ...
    if emps:
        for emp in emps:
            c1, c2, c3 = st.columns([1, 3, 1]) # Adjusted column widths
            with c1:
                if emp[3]: 
                    st.image(emp[3], width=80) # Fixed small size!
                else:
                    st.info("No Photo")
            with c2:
                st.markdown(f"#### {emp[1]}")
...
# ==========================================
# TAB 6: History View (Grouped by Employee for Cleaner Look)
# ==========================================
with tab6:
    ...
    if len(history_df) > 0:
        # Group data by Employee for a cleaner presentation
        ...
        for emp in unique_emps:
            ...
            # Drop the 'සේවකයා' column for display so it doesn't repeat redundantly
...
with tab8:
    ...
    with st.expander("➕ අලුත් භාණ්ඩයක් ඇතුලත් කරන්න"):
        ...
        new_prod_photo = st.file_uploader("පින්තූරය තෝරන්න (Gallery):", type=["jpg", "png", "jpeg"], key="prod_up")
